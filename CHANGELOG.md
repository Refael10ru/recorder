...
- `RecordTargets` in `targets.py`: global lifecycle manager owning recorder mode, per-test store, and player registry. Plugin creates it; `record_class`, `record_function`, and `@record` all reach it via `get_targets()`.
- `get_targets()` / `_set_targets()` in `targets.py`: accessor and installer for the session-global `RecordTargets` instance.

### Changed

- `plugin.py` is now a thin pytest adapter: only registers hooks and delegates to `RecordTargets`. `Controller` class removed.
...
- Player consumption is now asserted at `RecordTargets.end_test` (after teardown) instead of at `record_class.__exit__`, so fixture teardown method calls are included in the recording window. `test_play_underuse_raises_at_block_exit` renamed `test_play_underuse_raises_at_test_end`.
- `_BlockSource` shim removed from `targets.py` — player lifecycle is now fully owned by `RecordTargets`.
