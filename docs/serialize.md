...
| `encode_exception` | `(exc: BaseException) -> object` | Encode an exception; validates pickle round-trip at record time |
...
`_wrap_blob(blob)` constructs the envelope dict. Both `encode` and
`encode_exception` use it so the envelope format is defined in one place.
...
`decode` returns `Any`, not `object`. `pickle.loads` is typed as returning `Any`
and callers like `raise decode(exc_blob)` require the type to be narrowable to
`BaseException`. `object` is not raiseable in mypy's `[misc]` check; `Any` is,
