...
### Controller — extends StoreSource
...
**`test_id()`** — returns `self._nodeid`. Changes on every `begin_test`, so
`PlayerProxy._maybe_reload` detects the test boundary and reloads events.
...
**`register_player(player)`** — appends to `_players`. Called by
`PlayerProxy._maybe_reload` on each test boundary so `end_test` can assert full
consumption.
